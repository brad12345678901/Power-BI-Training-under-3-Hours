# Power BI Training under 3 Hours

This Git Repostiry was created to train myself, gain knowledge and technical skills on using Power BI while developing my future going into a Data Profession. Following files the things i did throughout the session while learning Power BI

https://www.youtube.com/watch?v=I0vQ_VLZTWg --> Video Source


## Here's the result of my Final Project
<img width="2079" height="1167" alt="Screenshot 2026-05-23 224415" src="https://github.com/user-attachments/assets/14b03a3a-e41a-4f18-a274-7d56b670c60e" />

An actual survey results conducted by the Author of the Video, basically a survey about data analytics as a profession and what people think about it.

## Observations that i seen based on the Visualization i created so far:

- **Data Scientist** has the highest average salary amongst all other professions mentioned
- **Python** is generally the most Favored Language to use when it comes to data analyzing
- Respondents are mostly not happy with the Salary they get with their jobs
- Respondents Work Life Balance is Ok for the most part
- Majority of the respondents think they neither had difficulties or gotten into a data profession with ease. Some thinks it was difficult for them getting in while others got into data profession easily


